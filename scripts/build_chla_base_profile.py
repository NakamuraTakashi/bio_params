...
from bio_params.base_profile import build_base_profile
...
def main() -> int:
    ...
    base = build_base_profile(df["Chla_surf"].to_numpy(), df["depth"].to_numpy(),
                              df["Chla_rel"].to_numpy())
    OUT_JSON.parent.mkdir(parents=True, exist_ok=True)
    OUT_JSON.write_text(json.dumps(base.to_dict(), indent=2))
    print(f"saved base table -> {OUT_JSON}")
